...
# 상대방으로부터 받는 데이터 #
def consoles():
    global enemy_to_x, enemy_to_y, enemy_left, running, iswin, game_time
    while True:
        ...
        if(message.decode()=='left'):
            ...
            iswin = True
            running = False
        elif('초' in message.decode()):
            string = message.decode()
            length = len(string)
            game_time = int(string[:(length-1)])

# 소켓 통신 연결 #
def accept():
    global client, server, addr, thr
...
# 충돌 처리 #
def collider():
    global sequence, character_idle_list_1, character_idle_list_2, my_x, my_y, ball_x, ball_y, left, running, iswin, thr
    if sequence<18:
        if left == False:
